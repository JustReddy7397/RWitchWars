package ga.justreddy.wiki.rwitchwars.dependency;
...
import ga.justreddy.wiki.rwitchwars.dependency.util.Urls;
import ga.justreddy.wiki.rwitchwars.dependency.util.Xmls;
...
public class DLoader {
  ...
  public void onLoad() throws ExceptionInInitializerError {
    if (!working) return;
    Bukkit.getConsoleSender().sendMessage(ChatColor.GREEN + "[RWitchWars] Loading dependencies...");
    dependencyFolder = new File("plugins/RWitchWars/libs");
    ...
    load(
        ...
        () -> {
          Bukkit.getConsoleSender()
              .sendMessage(
                  ChatColor.GREEN
                      + "[RWitchWars] Successfully downloaded the dependency "
                      + dependency.getName());
        });
  }
  ...
  public static void log(Level level, String... message) {
    Logger logger = RWitchWars.getWitchWars().getLogger();
